...
def train_gans(x_sampler, model_root, mode_name, netG, netD, args):
    ...
    d_median_loss_plot = plot_scalar(name = "d_median_loss", env= mode_name, rate = args.display_freq)
    g_median_loss_plot = plot_scalar(name = "g_median_loss", env= mode_name, rate = args.display_freq)
    ...
    for epoch in range(args.maxepoch):
        data_iter = iter(x_sampler)
        batch_count = 0
        i = 0
        while i < len(x_sampler):
            Diters = args.ncritic
            j = 0
            while j < args.ncritic and i < len(x_sampler):
                # (1) Update D network
                ...
                d_losses.append(d_loss.cpu().data.numpy().mean())
            ...
            if batch_count % args.display_freq == 0:
                ...
                save = (batch_count % args.save_freq == 0)
                z1   = z_sampler(batch_size, args.noise_dim)
                samples = netG(z1).cpu().data.numpy()
                imgs = save_images(
                            samples,
                            os.path.join(args.save_folder,'samples_{}.jpg'.format(batch_count) ),save=save,dim_ordering = 'th'
                            )
                print(samples.shape)
